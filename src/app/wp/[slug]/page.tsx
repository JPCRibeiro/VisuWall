...
async function WallpaperDetails({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-6">
      ...
      {/* Coluna de Detalhes (Ocupa 1/3 no desktop) */}
      <div className="space-y-6">
        <div className="bg-zinc-900/50 p-6 rounded-xl border border-zinc-800/50 backdrop-blur-sm">
          ...
          <div className="space-y-3">
            <Button className="w-full bg-white text-black hover:bg-zinc-200" asChild>
              <a href={wallpaper.image_url} target="_blank" rel="noopener noreferrer" download>
        ...
        <div className="space-y-4">
            <h3 className="text-sm font-medium text-zinc-400 uppercase tracking-wider">Detalhes</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="text-zinc-500">Resolução</div>
                <div className="text-zinc-300 text-right">{wallpaper.width} x {wallpaper.height}</div>
                
                <div className="text-zinc-500">Categoria</div>
                <div className="text-zinc-300 text-right capitalize">{wallpaper.category}</div>
